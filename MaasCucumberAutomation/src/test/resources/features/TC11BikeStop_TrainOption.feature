Feature: Validate Bike Stop route for Train option

@android
Scenario Outline:  Verify android app Bike Stop for Train option
...
@iOS  
 Scenario Outline:  Verify iOS app Bike Stop for Train option
  Given An anonymous user open the application and go to Bike stop
  When I Enter <Address> in Address field
  Then expect to see Route Button
  And click on Route Button 
